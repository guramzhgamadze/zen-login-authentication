...
function fauth_admin_register_settings(): void {
    ...
    // Rate limiting
    register_setting( 'zen-login-authentication', 'fauth_rate_limit',        [ 'sanitize_callback' => 'absint', 'type' => 'integer', 'autoload' => false ] );
    register_setting( 'zen-login-authentication', 'fauth_rate_limit_window', [ 'sanitize_callback' => 'absint', 'type' => 'integer', 'autoload' => false ] );
    ...
    foreach ( $rl_actions as $action ) {
        register_setting( 'zen-login-authentication', "fauth_rl_enabled_{$action}", [ 'sanitize_callback' => 'absint', 'type' => 'integer', 'autoload' => false ] );
        register_setting( 'zen-login-authentication', "fauth_rl_max_{$action}",     [ 'sanitize_callback' => 'absint', 'type' => 'integer', 'autoload' => false ] );
    }
    register_setting( 'zen-login-authentication', 'fauth_lostpassword_count_all', [ 'sanitize_callback' => 'absint', 'type' => 'integer', 'autoload' => false ] );

    // Subscriber post-login redirect: page slug/path or full URL; empty = site home.
    register_setting( 'zen-login-authentication', 'fauth_subscriber_redirect', [ 'sanitize_callback' => 'fauth_sanitize_redirect_target', 'type' => 'string', 'autoload' => false ] );

    // Sign in with Google (v1.5.0).
    register_setting( 'zen-login-authentication', 'fauth_google_enabled',            [ 'sanitize_callback' => 'absint',              'type' => 'integer', 'autoload' => false ] );
    register_setting( 'zen-login-authentication', 'fauth_google_client_id',          [ 'sanitize_callback' => 'sanitize_text_field', 'type' => 'string',  'autoload' => false ] );
    register_setting( 'zen-login-authentication', 'fauth_google_client_secret',      [ 'sanitize_callback' => 'fauth_sanitize_google_secret', 'type' => 'string', 'autoload' => false ] );
    register_setting( 'zen-login-authentication', 'fauth_google_allow_registration', [ 'sanitize_callback' => 'absint',              'type' => 'integer', 'autoload' => false ] );

    // Slugs
    $slug_actions = [ 'login', 'logout', 'register', 'lostpassword', 'resetpass', 'account' ];
    foreach ( $slug_actions as $action ) {
        register_setting( 'zen-login-authentication', "fauth_slug_{$action}", [ 'sanitize_callback' => 'sanitize_title', 'type' => 'string', 'autoload' => false ] );
    }
    ...
    foreach ( $widget_keys as $widget ) {
        register_setting( 'zen-login-authentication', "fauth_widget_enabled_{$widget}", [ 'sanitize_callback' => 'absint', 'type' => 'integer', 'autoload' => false ] );
    }

    // Login-activity log (v1.7.0).
    register_setting( 'zen-login-authentication', 'fauth_activity_log_enabled',  [ 'sanitize_callback' => 'absint', 'type' => 'integer', 'autoload' => false ] );
    register_setting( 'zen-login-authentication', 'fauth_activity_retention_days', [ 'sanitize_callback' => 'absint', 'type' => 'integer', 'autoload' => false ] );
}
    ...
    <div class="fauth-admin">
        ...
        <form method="post" action="options.php">
            ...
            <div class="fauth-card">
                ...
                <?php
                $toggles = [
                    [ 'fauth_use_permalinks', __( 'Pretty URLs', 'zen-login-authentication' ), __( 'Use /login/ instead of ?action=login', 'zen-login-authentication' ), true ],
                    [ 'fauth_use_ajax', __( 'AJAX forms', 'zen-login-authentication' ), __( 'Submit forms without a page reload', 'zen-login-authentication' ), false ],
                    [ 'fauth_user_passwords', __( 'User-chosen passwords', 'zen-login-authentication' ), __( 'Show password field on registration', 'zen-login-authentication' ), false ],
                    [ 'fauth_auto_login', __( 'Auto-login', 'zen-login-authentication' ), __( 'Log users in automatically after registration', 'zen-login-authentication' ), false ],
                    [ 'fauth_honeypot', __( 'Honeypot protection', 'zen-login-authentication' ), __( 'Hidden field to catch spam bots', 'zen-login-authentication' ), true ],
                ];
                ...
                <div class="fauth-row">
                    ...
                    <div class="fauth-row-field">
                        <input type="text" name="fauth_subscriber_redirect" value="<?php echo esc_attr( (string) get_option( 'fauth_subscriber_redirect', '' ) ); ?>" placeholder="<?php esc_attr_e( 'e.g. dashboard  (leave empty for home page)', 'zen-login-authentication' ); ?>">
                ...
                <div class="fauth-row">
                    ...
                    <div class="fauth-row-field">
                        <?php if ( defined( 'FAUTH_GOOGLE_CLIENT_SECRET' ) ) : ?>
                            ...
                            <div class="fauth-hint"><?php esc_html_e( 'Defined as FAUTH_GOOGLE_CLIENT_SECRET in wp-config.php — manage it there.', 'zen-login-authentication' ); ?></div>
                        <?php else : ?>
                            <?php $fauth_has_secret = '' !== trim( (string) get_option( 'fauth_google_client_secret', '' ) ); ?>
                            <input type="password" name="fauth_google_client_secret" value="" autocomplete="new-password" placeholder="<?php echo esc_attr( $fauth_has_secret ? __( 'Saved — leave blank to keep the current secret', 'zen-login-authentication' ) : '' ); ?>">
                            <div class="fauth-hint"><?php esc_html_e( 'Stored encrypted (AES-256-GCM, keyed from your wp-config.php salts) and never shown again after saving. Tip: you can instead define FAUTH_GOOGLE_CLIENT_ID and FAUTH_GOOGLE_CLIENT_SECRET in wp-config.php to keep credentials out of the database entirely. If you rotate your salts, re-enter the secret.', 'zen-login-authentication' ); ?></div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="fauth-card">
                ...
                <div class="fauth-row">
                    ...
                    <div class="fauth-row-field">
                        <input type="number" name="fauth_rate_limit_window" value="<?php echo esc_attr( (string) get_option( 'fauth_rate_limit_window', 15 ) ); ?>" min="1" max="1440"> <?php esc_html_e( 'minutes', 'zen-login-authentication' ); ?>
                    </div>
                </div>
            </div>
            ...
            <div class="fauth-card">
                ...
                <div class="fauth-row">
                    ...
                    <div class="fauth-row-field">
                        <input type="number" name="fauth_activity_retention_days" value="<?php echo esc_attr( (string) get_option( 'fauth_activity_retention_days', 30 ) ); ?>" min="0" max="365"> <?php esc_html_e( 'days', 'zen-login-authentication' ); ?>
